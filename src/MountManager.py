...
def getProcPartitions(List):
	...
	with open("/proc/partitions", "r") as f:
		for line in f.readlines():
			...
			# print "[MountManager0] device = %s devmajor = %s devminor = %s" % (device, devmajor, devminor)
			if devmajor == "major":
				continue
			...
			if devMajor == 8:
				...
				if SystemInfo["HasHiSi"] and path.exists("/dev/sda4") and re.search("sd[a][1-4]", device):	# sf8008 using SDcard for slots ---> exclude
					continue
			partition = device		# so we now have a partition
			if partition in partitions:	# if already in partition list ignore
				continue
			buildPartitionInfo(partition, List)
			partitions.append(partition)
			# print "[MountManager5] partitions = %s" % partitions
...
def buildPartitionInfo(partition, List):
	...
	# print "[MountManager1]partition: %s" %partition
	# print "[MountManager1]device: %s" %device
	# print "[MountManager1]physicalDevice:%s" %physicalDevice
	...
	if path.exists(resolveFilename(SCOPE_ACTIVE_SKIN, "vixcore/dev_%s.png" % pngType)):
		mypixmap = resolveFilename(SCOPE_ACTIVE_SKIN, "vixcore/dev_%s.png" % pngType)
	else:
...
class VIXDevicesPanel(Screen):
	...
	def Mount(self):
		...
		if sel:
			...
			if exitStatus != 0:
				self.session.open(MessageBox, _("Mount failed for %s, error code = %s." % (sel, exitStatus)), MessageBox.TYPE_INFO, timeout=5)
			self.setTimer()
	...
	def Unmount(self):
		...
		if sel:
			...
			if exitStatus == 0:
				...
				self.session.open(MessageBox, _("Cannot un-mount the partition %s; make sure it is not being used for SWAP or record/timeshift paths." % mountp), MessageBox.TYPE_INFO)
				return -1
